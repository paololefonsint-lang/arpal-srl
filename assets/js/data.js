/* ==========================================================================
   ARPAL — contenuti strutturati (timeline, materiali, gallerie)
   ========================================================================== */

window.ARPAL_TIMELINE = [
  { year:"1979", title:"Nasce ARPAL", text:"Nasce ARPAL, dando inizio a un percorso dedicato alle tecnologie, ai materiali e alle lavorazioni per il settore edilizio." },
  { year:"1981", title:"ARPAL diventa S.r.l.", text:"ARPAL si trasforma in società a responsabilità limitata. Con la famiglia Arduino alla guida dell'azienda prende forma una nuova fase di sviluppo e consolidamento." },
  { year:"1984", title:"L'attività si amplia", text:"ARPAL amplia il proprio campo di attività, ponendo le basi per una progressiva crescita delle competenze nel settore edilizio." },
  { year:"1986", title:"L'apertura a Finale Ligure", text:"ARPAL amplia la propria presenza territoriale con l'apertura dell'unità di Finale Ligure, portando la propria esperienza anche sul territorio ligure." },
  { year:"1996", title:"L'apertura di Poirino", text:"Con l'apertura della sede di Poirino, ARPAL rafforza la propria struttura operativa e consolida la presenza sul territorio piemontese." },
  { year:"2001", title:"Dalla posa a un'attività edilizia più completa", text:"ARPAL amplia ulteriormente il proprio raggio d'azione: all'installazione e alla posa si affiancano nuove competenze nella produzione, nelle costruzioni e nelle ristrutturazioni, consolidando il ruolo dell'azienda nel settore edilizio." },
  { year:"2018", title:"Nascono lattoneria e rivestimenti", text:"ARPAL amplia le proprie competenze entrando nel settore della lattoneria e dei rivestimenti metallici. All'esperienza maturata nelle impermeabilizzazioni si affiancano nuove lavorazioni dedicate a coperture, facciate e rivestimenti architettonici." },
  { year:"2021", title:"Il passaggio generazionale", text:"La seconda generazione della famiglia Arduino assume la guida di Arpal: il passaggio generazionale rafforza la continuità familiare dell'impresa, che prosegue il percorso iniziato oltre quarant'anni prima." },
  { year:"2024", title:"Cresce la struttura aziendale", text:"Negli ultimi anni ARPAL continua a investire nelle proprie competenze e nella crescita della struttura aziendale, ampliando il proprio parco veicolare." },
  { year:"2025", title:"Una squadra sempre più specializzata", text:"La crescita del settore lattoneria compie un nuovo passo: ARPAL può contare su un team di lattonieri specializzati e certificati CERTIS, a conferma dell'investimento nella formazione e nella qualità delle lavorazioni." },
];

window.ARPAL_MATERIALS = [
  {
    id:"alluminio", tag:"Metalli", name:"Alluminio",
    lede:"Leggerezza, resistenza e grande versatilità progettuale per coperture, facciate e rivestimenti.",
    text:[
      "L'alluminio è un materiale particolarmente adatto alla realizzazione di <a href=\"coperture.html\">coperture</a>, <a href=\"facciate.html\">facciate</a> e <a href=\"rivestimenti-metallici.html\">rivestimenti architettonici</a>, grazie alla combinazione tra leggerezza, resistenza e grande versatilità progettuale.",
      "Uno dei suoi principali punti di forza è il peso ridotto, caratteristica che ne facilita la lavorazione e la posa e che lo rende particolarmente interessante anche negli interventi di ristrutturazione, dove è importante limitare i carichi aggiuntivi sulle strutture esistenti.",
      "A contatto con l'aria, l'alluminio sviluppa naturalmente uno strato superficiale di ossido che contribuisce a proteggerlo dalla corrosione. È inoltre estremamente lavorabile e può essere sagomato, piegato e adattato a forme architettoniche anche complesse.",
      "La disponibilità di numerose colorazioni, finiture, formati e sistemi di posa consente di utilizzarlo sia per architetture contemporanee sia per interventi di recupero, lasciando grande libertà nella progettazione dell'involucro edilizio."
    ],
    benefits:["Leggerezza, particolarmente vantaggiosa nelle ristrutturazioni","Elevata resistenza alla corrosione e agli agenti atmosferici","Lunga durata nel tempo","Ridotte esigenze di manutenzione","Grande lavorabilità e possibilità di sagomatura","Ampia scelta di forme, colori e finiture","Adatto a coperture, facciate e rivestimenti","Elevata riciclabilità del materiale","Si integra facilmente con legno, vetro, pietra e intonaco"]
  },
  {
    id:"rame", tag:"Metalli", name:"Rame",
    lede:"Il materiale più tradizionale e prestigioso: sviluppa nel tempo una patina protettiva dal carattere unico.",
    text:[
      "Il rame è uno dei materiali più tradizionali e prestigiosi utilizzati nell'architettura per la realizzazione di <a href=\"coperture.html\">coperture</a>, <a href=\"rivestimenti-metallici.html\">rivestimenti</a> e opere di lattoneria.",
      "La sua caratteristica più riconoscibile è la naturale trasformazione della superficie nel corso del tempo. Esposto agli agenti atmosferici, il rame modifica progressivamente il proprio colore fino alla formazione di una patina naturale protettiva, il cui aspetto varia in funzione delle condizioni ambientali e dell'esposizione.",
      "Questa evoluzione non rappresenta un deterioramento del materiale, ma contribuisce alla sua protezione e conferisce alle superfici un carattere unico e mutevole.",
      "Il rame è inoltre estremamente duttile e malleabile: può essere facilmente sagomato e adattato anche a coperture curve, dettagli complessi e particolari architettonici."
    ],
    benefits:["Eccezionale durata nel tempo","Naturale resistenza alla corrosione","Formazione di una patina protettiva","Aspetto estetico che evolve e acquista carattere nel tempo","Elevata malleabilità e lavorabilità","Ideale per dettagli e geometrie complesse","Ridotte esigenze di manutenzione","Non necessita normalmente di verniciature superficiali","Materiale riciclabile e recuperabile a fine vita"]
  },
  {
    id:"zinco-titanio", tag:"Metalli", name:"Zinco-titanio",
    lede:"Patina naturale protettiva e tonalità sobrie per l'architettura contemporanea.",
    text:[
      "Lo zinco-titanio è un materiale particolarmente apprezzato nell'architettura contemporanea per la realizzazione di <a href=\"coperture.html\">coperture</a>, <a href=\"facciate.html\">facciate</a>, <a href=\"rivestimenti-metallici.html\">rivestimenti</a> e opere di lattoneria.",
      "Una delle sue caratteristiche più interessanti è la capacità di sviluppare naturalmente una patina superficiale protettiva a contatto con l'atmosfera. Questo processo permette al materiale di evolvere nel tempo e contribuisce alla sua protezione dagli agenti atmosferici.",
      "La superficie assume tonalità naturali e sobrie che consentono allo zinco-titanio di integrarsi facilmente sia in edifici moderni sia in interventi di recupero e riqualificazione.",
      "La buona lavorabilità del materiale permette inoltre di realizzare aggraffature, piegature, raccordi e rivestimenti anche su geometrie articolate, offrendo ampia libertà nella progettazione."
    ],
    benefits:["Elevata durabilità","Formazione naturale di una patina protettiva","Ridotte esigenze di manutenzione","Elevata resistenza agli agenti atmosferici","Ottima lavorabilità e possibilità di sagomatura","Adatto a superfici e geometrie complesse","Estetica sobria, naturale e contemporanea","Materiale altamente riciclabile"]
  },
  {
    id:"rheinzink", tag:"Metalli", name:"RHEINZINK",
    lede:"Zinco-titanio di elevata qualità sviluppato specificamente per applicazioni architettoniche.",
    text:[
      "RHEINZINK è uno zinco-titanio di elevata qualità specificamente sviluppato per applicazioni architettoniche quali <a href=\"coperture.html\">coperture</a>, <a href=\"facciate.html\">facciate</a> e sistemi di smaltimento delle acque meteoriche.",
      "Come lo zinco-titanio naturale, il materiale sviluppa nel tempo una patina superficiale protettiva che contribuisce alla sua elevata durabilità e conferisce alle superfici un aspetto naturale e caratteristico.",
      "La sua elevata lavorabilità permette di adattarlo a geometrie complesse, realizzare coperture aggraffate, rivestimenti di facciata, dettagli, raccordi e numerosi elementi di lattoneria.",
      "Le differenti tipologie di superficie disponibili permettono inoltre di scegliere l'aspetto più adatto all'architettura dell'edificio, mantenendo la naturale evoluzione caratteristica dello zinco."
    ],
    benefits:["Lunghissima durata se correttamente progettato e posato","Formazione naturale di una patina superficiale protettiva","Ridotte esigenze di manutenzione","Elevata lavorabilità","Adatto a coperture, facciate e opere di lattoneria","Ideale anche per geometrie architettoniche complesse","Aspetto naturale ed elegante","Materiale altamente riciclabile"]
  },
  {
    id:"acciaio-inox", tag:"Metalli", name:"Acciaio inox",
    lede:"Resistenza, durabilità ed elevato valore estetico per rivestimenti e dettagli architettonici.",
    text:[
      "L'acciaio inox combina resistenza, durabilità ed elevato valore estetico, caratteristiche che ne fanno un materiale particolarmente indicato per <a href=\"rivestimenti-metallici.html\">rivestimenti</a> e dettagli architettonici destinati a mantenere le proprie prestazioni nel tempo.",
      "La presenza di elementi di lega, in particolare il cromo, consente all'acciaio inox di sviluppare una superficie naturalmente resistente alla corrosione. Per questo motivo può essere utilizzato anche in applicazioni esterne particolarmente esposte agli agenti atmosferici.",
      "Un altro elemento distintivo è l'ampia possibilità di personalizzazione estetica: superfici lucide, satinate, opache, spazzolate o strutturate permettono di ottenere effetti molto differenti.",
      "La combinazione tra resistenza meccanica e qualità estetica consente di realizzare facciate, portali, rivestimenti e dettagli caratterizzati da un'immagine contemporanea e particolarmente elegante."
    ],
    benefits:["Elevata resistenza alla corrosione","Grande durabilità nel tempo","Elevata resistenza meccanica","Ridotte esigenze di manutenzione","Ampia disponibilità di finiture superficiali","Estetica moderna e prestigiosa","Adatto ad applicazioni interne ed esterne","Materiale completamente riciclabile"]
  },
  {
    id:"pannelli-compositi", tag:"Sistemi", name:"Pannelli compositi",
    lede:"Soluzione versatile per facciate ventilate, rivestimenti, portali e intradossi.",
    text:[
      "I pannelli compositi in alluminio rappresentano una soluzione estremamente versatile per la realizzazione di <a href=\"facciate.html\">facciate</a> ventilate, <a href=\"rivestimenti-metallici.html\">rivestimenti</a>, portali, balconi, intradossi ed elementi architettonici.",
      "La loro struttura, composta generalmente da due strati esterni in alluminio abbinati a un nucleo interno, permette di ottenere elementi caratterizzati contemporaneamente da peso contenuto, elevata rigidità e ottima planarità.",
      "Uno dei maggiori vantaggi è la libertà progettuale. I pannelli possono essere tagliati, fresati, forati, piegati e sagomati in funzione delle esigenze del progetto, consentendo di realizzare elementi su misura e geometrie molto differenti.",
      "L'ampia scelta di colorazioni e finiture consente inoltre di caratterizzare in modo significativo l'estetica dell'edificio, mantenendo superfici regolari e omogenee anche su facciate di grandi dimensioni."
    ],
    benefits:["Peso contenuto rispetto alle dimensioni dei pannelli","Elevata rigidità e stabilità","Ottima planarità delle superfici","Buona resistenza agli agenti atmosferici","Grande libertà progettuale","Facilmente lavorabili: taglio, fresatura, foratura, piegatura","Ampia gamma di colori e finiture","Particolarmente indicati per facciate ventilate"]
  },
  {
    id:"corten", tag:"Metalli", name:"Corten",
    lede:"Acciaio auto patinabile dall'estetica unica: il colore evolve nel tempo con le tonalità calde della ruggine.",
    text:[
      "Il Corten, o più in generale acciaio auto patinabile, è un materiale fortemente caratterizzante dal punto di vista architettonico.",
      "La sua particolarità consiste nella capacità di sviluppare, quando esposto alle condizioni ambientali appropriate, una patina superficiale dall'aspetto naturale e mutevole, caratterizzata dalle tipiche tonalità calde che vanno dall'arancio al bruno scuro.",
      "Questa patina non ha soltanto una funzione estetica: nelle corrette condizioni di esposizione forma uno strato aderente che rallenta il processo di corrosione del materiale sottostante.",
      "Il risultato è una superficie viva, il cui colore e aspetto cambiano progressivamente nel corso del tempo e che si inserisce con particolare efficacia sia nei contesti urbani sia negli ambienti naturali.",
      "Per queste caratteristiche viene spesso utilizzato per <a href=\"facciate.html\">facciate</a>, <a href=\"rivestimenti-metallici.html\">rivestimenti</a>, portali ed elementi architettonici nei quali il materiale stesso deve diventare protagonista del progetto."
    ],
    benefits:["Estetica unica e fortemente riconoscibile","Formazione naturale di una patina protettiva","Colore e superficie che evolvono nel tempo","Elevata resistenza meccanica","Buona resistenza alla corrosione atmosferica","Possibilità di utilizzo senza verniciatura in molte applicazioni","Ideale per facciate e rivestimenti contemporanei","Si integra bene con vetro, legno, calcestruzzo e pietra"]
  },
  {
    id:"membrane-bituminose", tag:"Impermeabilizzazione", name:"Membrane bituminose",
    lede:"La soluzione più consolidata e versatile per i sistemi impermeabili in edilizia.",
    text:[
      "Le membrane bituminose rappresentano una delle soluzioni più consolidate e versatili per la realizzazione di sistemi impermeabili in edilizia, alla base delle <a href=\"impermeabilizzazioni.html\">impermeabilizzazioni</a> Arpal.",
      "Grazie all'evoluzione dei bitumi modificati con polimeri e delle armature interne, le membrane moderne permettono di ottenere sistemi capaci di combinare impermeabilità, flessibilità, stabilità e resistenza meccanica.",
      "Possono essere utilizzate in numerosi ambiti: coperture piane e inclinate, terrazzi, strutture interrate, fondazioni, giardini pensili e molte altre applicazioni nelle quali è necessario impedire la penetrazione dell'acqua.",
      "Sono inoltre disponibili prodotti specifici per esigenze particolari, come membrane antiradice, barriere al vapore o membrane autoprotette."
    ],
    benefits:["Elevata capacità impermeabilizzante","Tecnologia consolidata e ampiamente utilizzata","Grande versatilità di applicazione","Possibilità di realizzare sistemi multistrato","Buona resistenza meccanica","Possibilità di seguire raccordi, risvolti e particolari costruttivi","Versioni specifiche per coperture, interrati e giardini pensili","Elevata durabilità se correttamente progettata e posata"]
  },
  {
    id:"epdm", tag:"Impermeabilizzazione", name:"EPDM",
    lede:"Membrana sintetica elastomerica ad alta elasticità, usata anche per laghi e bacini artificiali.",
    text:[
      "L'EPDM è una membrana sintetica elastomerica utilizzata per la realizzazione di impermeabilizzazioni caratterizzate da elevata elasticità, durabilità e resistenza agli agenti atmosferici.",
      "Una delle caratteristiche principali del materiale è la capacità di mantenere la propria flessibilità anche in presenza di importanti variazioni di temperatura, adattandosi ai movimenti del supporto e alle geometrie dell'opera.",
      "L'EPDM presenta un'ottima resistenza ai raggi UV, all'ozono e all'invecchiamento dovuto all'esposizione atmosferica, caratteristiche che lo rendono particolarmente interessante per applicazioni di lunga durata.",
      "Arpal utilizza questa tecnologia anche per l'impermeabilizzazione di <a href=\"laghi-artificiali.html\">laghi e bacini artificiali</a>, dove elasticità, continuità del rivestimento e capacità di adattarsi alla conformazione del fondo sono caratteristiche fondamentali."
    ],
    benefits:["Elevata elasticità e flessibilità","Grande capacità di adattarsi alle geometrie del supporto","Ottima resistenza ai raggi UV e all'ozono","Elevata resistenza agli agenti atmosferici","Buon comportamento alle variazioni di temperatura","Lunga durata nel tempo","Teli di grandi dimensioni, meno giunzioni","Adatto sia a coperture sia a laghi e bacini artificiali"]
  },
  {
    id:"isolanti", tag:"Isolamento", name:"Isolanti",
    lede:"Prestazioni energetiche e comfort abitativo attraverso un corretto sistema di isolamento.",
    text:[
      "Un corretto sistema di isolamento è fondamentale per migliorare le prestazioni energetiche dell'involucro edilizio e aumentare il comfort degli ambienti interni.",
      "Arpal utilizza differenti tipologie di materiali isolanti, selezionandoli in funzione delle caratteristiche della <a href=\"coperture.html\">copertura</a> o della <a href=\"facciate.html\">facciata</a>, delle prestazioni richieste dal progetto e della stratigrafia all'interno della quale devono essere inseriti.",
      "L'isolamento permette di limitare lo scambio di calore tra interno ed esterno, contribuendo a mantenere temperature più stabili durante l'inverno e l'estate e riducendo il fabbisogno energetico per riscaldamento e raffrescamento.",
      "Per le coperture piane possono inoltre essere utilizzati pannelli isolanti sagomati a pendenza, capaci di integrare la funzione termica con la corretta gestione del deflusso delle acque."
    ],
    benefits:["Miglioramento dell'isolamento termico dell'edificio","Maggiore comfort in inverno e in estate","Riduzione delle dispersioni energetiche","Possibile riduzione dei consumi per riscaldamento/raffrescamento","Materiali e densità differenti in funzione del progetto","In alcune tipologie, migliori prestazioni acustiche","In alcune tipologie, elevata resistenza al fuoco","Integrazione con impermeabilizzazione e rivestimento"]
  }
];

window.ARPAL_GALLERIES = {
  coperture:[
    {src:"assets/img/coperture/cornicione-alluminio-1.jpg", alt:"Cornicione di copertura in alluminio nero opaco"},
    {src:"assets/img/coperture/portale-corten-1.jpg", alt:"Portale con finitura corten integrato in copertura"},
    {src:"assets/img/coperture/cornicione-alluminio-2.jpg", alt:"Dettaglio cornicione in alluminio nero opaco"},
    {src:"assets/img/coperture/portale-corten-2.jpg", alt:"Portale corten, vista di dettaglio"}
  ],
  facciate:[
    {src:"assets/img/coperture/cornicione-alluminio-1.jpg", alt:"Facciata con cornicione tecnico in alluminio nero opaco"},
    {src:"assets/img/coperture/cornicione-alluminio-3.jpg", alt:"Dettaglio dell'involucro in alluminio"},
    {src:"assets/img/coperture/portale-corten-1.jpg", alt:"Portale in corten integrato in facciata"},
    {src:"assets/img/coperture/portale-corten-3.jpg", alt:"Dettaglio finitura corten su facciata"}
  ],
  rivestimenti:[
    {src:"assets/img/coperture/portale-corten-2.jpg", alt:"Rivestimento metallico effetto corten, portale d'ingresso"},
    {src:"assets/img/coperture/portale-corten-3.jpg", alt:"Portale con rivestimento in acciaio corten"},
    {src:"assets/img/coperture/cornicione-alluminio-2.jpg", alt:"Rivestimento tecnico in alluminio nero opaco"},
    {src:"assets/img/coperture/cornicione-alluminio-3.jpg", alt:"Rivestimento in alluminio, vista d'insieme"}
  ],
  lineavita:[
    {src:"assets/img/lineavita/lineavita-2.jpg", alt:"Linea vita su copertura metallica aggraffata"},
    {src:"assets/img/lineavita/lineavita-1.jpg", alt:"Punto di ancoraggio linea vita, dettaglio"},
    {src:"assets/img/lineavita/lineavita-3.jpg", alt:"Sistema anticaduta installato in copertura"}
  ],
  impermeabilizzazioni:[
    {src:"assets/img/impermeabilizzazioni/guaina-bituminosa-1.jpg", alt:"Posa di guaina bituminosa ceramizzata"},
    {src:"assets/img/impermeabilizzazioni/guaina-nera-1.jpg", alt:"Guaina ceramizzata nera su copertura"},
    {src:"assets/img/impermeabilizzazioni/copertura-1.jpg", alt:"Copertura impermeabilizzata"},
    {src:"assets/img/impermeabilizzazioni/copertura-pendente-1.jpg", alt:"Copertura pendente impermeabilizzata"}
  ],
  laghi:[
    {src:"assets/img/laghi/bacino-epdm-1.jpg", alt:"Posa membrana EPDM in bacino artificiale"},
    {src:"assets/img/laghi/bacino-epdm-2.jpg", alt:"Bacino artificiale in EPDM, vista d'insieme"},
    {src:"assets/img/laghi/bacino-epdm-3.jpg", alt:"Dettaglio membrana EPDM in bacino artificiale"},
    {src:"assets/img/laghi/bacino-epdm.mp4", alt:"Video: realizzazione bacino artificiale in EPDM", video:true, poster:"assets/img/laghi/bacino-epdm-2.jpg"}
  ]
};
